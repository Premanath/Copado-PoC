({
    ...
    applyAttribute: function(component, entityId, segmentCode, attributeNameToBeApplied) {
        var i;
    ...
    unapplyAttribute: function(component, entityId, attributeNameToBeUnapplied) {
        var i, j;
        var appliedAttributes = component.get("v.appliedAttributes");
        var attributeNameToBeUnappliedSegmentCode;
        for (i=0; i<appliedAttributes.length; i+=1) {
            if (attributeNameToBeUnapplied === appliedAttributes[i].Name) {
                attributeNameToBeUnappliedSegmentCode = appliedAttributes[i].SegmentCode;
                break;
            }
        }
        var action = component.get("c.removeSegment");
        action.setParams({"clientId" : entityId,
                          "segmentCode" : attributeNameToBeUnappliedSegmentCode,
                          "storyId" : ""});
        ...
        action.setCallback(this, function(response) {
            ...
            if (component.isValid() && state === "SUCCESS") {
                ...
                //alert("unapplyAttribute done!!!");
                var attributeUnapplied;
                for (j=0; j<appliedAttributes.length; j+=1) {
                    if (attributeNameToBeUnapplied === appliedAttributes[j].Name) {
