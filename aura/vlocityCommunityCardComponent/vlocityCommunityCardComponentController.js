        ...
        var actionHandler = $A.getCallback(function(data) {
            ...
            if (message.message == 'actionLauncher:windowopen') {
                switch(message.action.type) {
                    case 'Custom': 
                    case 'OmniScript':  helper.handleUrl(cmp, message.action.url, message.action, message.contextId, message.sObjectType);
                                        break;
                    default:            helper.handleRegularAction(cmp, message.action, message.contextId, message.sObjType);
                }
            }
        });
        var config = {
